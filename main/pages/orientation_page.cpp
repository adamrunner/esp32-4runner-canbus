...
typedef struct {
    ...
    lv_obj_t *diag_lateral_g_value;
    lv_obj_t *diag_yaw_rate_value;
    lv_obj_t *diag_steering_angle_value;
    // Broadcast orientation data (from 0x024 / 0x025)
    lv_obj_t *bcast_lateral_g_value;
    lv_obj_t *bcast_yaw_rate_value;
    lv_obj_t *bcast_steering_angle_value;
...
static void orientation_page_on_create(dm_page_t *page, lv_obj_t *parent)
{
    ...
    // Live orientation data (from 0x7B0 PID 0x47)
    lv_obj_t *card = create_metric_card(grid, "Lat G (Diag)", &data->diag_lateral_g_value);
    lv_obj_set_size(card, LV_PCT(48), 100);

    card = create_metric_card(grid, "Lat G (Bcast)", &data->bcast_lateral_g_value);
    lv_obj_set_size(card, LV_PCT(48), 100);

    card = create_metric_card(grid, "Yaw (Diag)", &data->diag_yaw_rate_value);
    lv_obj_set_size(card, LV_PCT(48), 100);

    card = create_metric_card(grid, "Yaw (Bcast)", &data->bcast_yaw_rate_value);
    lv_obj_set_size(card, LV_PCT(48), 100);

    card = create_metric_card(grid, "Steer (Diag)", &data->diag_steering_angle_value);
    lv_obj_set_size(card, LV_PCT(48), 100);

    card = create_metric_card(grid, "Steer (Bcast)", &data->bcast_steering_angle_value);
    lv_obj_set_size(card, LV_PCT(48), 100);
...
static void orientation_page_on_update(dm_page_t *page)
{
    ...
    lv_label_set_text(data->diag_lateral_g_value, buf);

    if (snap.bcast_kinematics_valid) {
        snprintf(buf, sizeof(buf), "%.2f g", snap.bcast_lateral_g);
    } else {
        snprintf(buf, sizeof(buf), "--");
    }
    lv_label_set_text(data->bcast_lateral_g_value, buf);
    ...
    lv_label_set_text(data->diag_yaw_rate_value, buf);

    if (snap.bcast_kinematics_valid) {
        snprintf(buf, sizeof(buf), "%.1f d/s", snap.bcast_yaw_rate_deg_sec);
    } else {
        snprintf(buf, sizeof(buf), "--");
    }
    lv_label_set_text(data->bcast_yaw_rate_value, buf);
    ...
    lv_label_set_text(data->diag_steering_angle_value, buf);

    if (snap.bcast_steer_angle_valid) {
        snprintf(buf, sizeof(buf), "%.1f", snap.bcast_steering_angle_deg);
    } else {
        snprintf(buf, sizeof(buf), "--");
    }
    lv_label_set_text(data->bcast_steering_angle_value, buf);
